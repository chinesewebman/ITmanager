...
// W1：`catch { return MOCK_RULES }` + `data: rules = MOCK_RULES` 双重兜底已删除。
// 原写法让 isError 恒 false —— 接口挂了页面照常列出虚构的抑制规则，运维会以为
...
export function AlertSuppressions() {
  ...
  const { data, isLoading, isError, error, refetch } = useApiQuery<AlertSuppression[]>(
    ...
    async () => {
      const items = await apiGet<AlertSuppression[]>('/alert-suppressions')
