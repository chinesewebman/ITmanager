...
describe('useAlertStore', () => {
  ...
  it('初始 stats 4 个 counter 都是 0', () => {
    ...
    expect(s.stats).toEqual({ total: 0, problem: 0, acknowledged: 0, resolved: 0 })
  })
  ...
  it('setStats 4 字段统计', () => {
    useAlertStore.getState().setStats({ total: 100, problem: 10, acknowledged: 5, resolved: 85 })
    expect(useAlertStore.getState().stats).toEqual({ total: 100, problem: 10, acknowledged: 5, resolved: 85 })
  })

  it('setFilters 设 status/severity', () => {
    useAlertStore.getState().setFilters({ status: 'problem', severity: '5' })
    expect(useAlertStore.getState().filters).toEqual({ status: 'problem', severity: '5' })
  })
})
...
describe('useAssetStore', () => {
  ...
  it('setFilters 支持 site_id/type/status', () => {
    useAssetStore.getState().setFilters({ site_id: 's1', type: 'server', status: 'active' })
    expect(useAssetStore.getState().filters).toEqual({ site_id: 's1', type: 'server', status: 'active' })
  })
})
...
describe('useTicketStore', () => {
  ...
  it('setFilters 支持 status/priority', () => {
    ...
    expect(useTicketStore.getState().filters).toEqual({ status: 'open', priority: 'high' })
  })
})
