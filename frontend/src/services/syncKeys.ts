...
//
// 本模块把这类「跨语言裸字符串约定」固化成 TS 常量 + 跨语言契约测试
// (syncKeys.test.ts) 守住「两端必须漂移同步 + Settings.tsx 不可写回裸字符串」。
//
...
/**
 * Zabbix 源侧 0/1 截断标志 (data.synced 字典的键名)。
 *
 * 语义 (M27/D-6 + M33/D-6)：
 ...
 *
 * 契约：值必须与 backend/internal/integration/sync_keys.go 的 KeyZabbixTruncated 一致。
 * 契约测试：syncKeys.test.ts SyncKeys_CrossLangWithGo。
 */
