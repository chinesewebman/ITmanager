...
### 8.6 F 项（G-34 / G-35）— **不修**，降级为「已登记、待独立模块」（2026-09-11）

**为什么 §0 的缩放决策在这里生效**：F 项的修法（把 `redact` 规则 3 拆成「引号形态 / 裸值形态」
两条，或换成小型状态机）会**重写 G-28 刚收敛的三条规则**，而 G-34 与 G-35 的边界**互相咬合**
（规则 3 一旦「知道自己在 URL 里」，G-34 的三条"漏"用例期望值同时移动）。在一个已经含 5 项
独立修复的批次里顺带做，等于把两个高风险面叠在一起 —— 按 §0 的**中止条件**，拆出去独立一轮。

**本轮的实际交付是「把边界测准」**（`redact.Text` 真实输出，probe 实测、非推演）：

| 输入 | `Text()` 实际输出 | 判定 |
|---|---|---|
| `password=&SECRET` | `password=&SECRET` | **漏**（G-34，规则 3） |
| `password=;SECRET` | `password=;SECRET` | **漏**（G-34，规则 3） |
| `Authorization: Bearer "SECRET` | 原样 | **漏**（G-34，规则 2） |
| `token="S` | `token="***` | **不漏** —— rev1 曾误判为漏（规则 3 的值类已含可选引号组） |
| `http://token:8080/x` | `http://token:***` | **过度**（G-35，非泄漏） |

**交接前置**：下轮开工前先把上表 5 组固化成 P1 用例 —— 其中 3 条是"漏"的期望行为，另 2 条是
**边界锁定**（防止修 G-35 时把 `token="S` 或端口那两条改坏而无人发现）。

### 8.7 台账（M28-G）— 已交付 2026-09-11

- `TODO.md`：结案 5 项（G-8 / G-11 / G-12② / G-29 / G-38），逐项写明「已交付 + 文件:行 + 变异编号」；
  **事实更正**：G-8 的判据键 `auth_type` → `api_key_id`（`auth_type` 只是 SQL 列名）；
  G-29 的目标章节 §8.4.3 → **§8.4.4**（仓库根 `08-部署运维.md`，§8.4.3 是 log.level/SQL 日志）。
- `TODO.md` 新增：**G-42**（白名单过宽 CIDR 无写入侧守卫，本轮刻意不做 + 为什么）、
  **G-43**（`apierr.Respond` 日志行裸拼 `URL.Path`，`%0d%0a` 可伪造日志行，实测复现，CWE-117）。
- `TODO.md` 改判：G-34 / G-35 由「未修」→「**已登记、待独立模块**」，附 §8.6 的实测边界表。
- `docs/TRAPS.md`：新增 T-52（安全控制两侧必须共享语义）、T-53（fail-open 守卫判据不能是
  「只有前置中间件才会设置的键」）、T-54（同一个 `URL.Path`：`%#v` 安全、裸拼接可被伪造），
  并登记 §五 索引。
- `CHANGELOG.md`：M28 条目 + **行为突变告知**（A 项修完后存量 CIDR 白名单条目开始生效，
  运维须复核 `ip_whitelist`；含 `0.0.0.0/0` 的具体意外场景）。
