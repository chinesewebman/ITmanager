...
| 批 2（M1/M2/M3+P4/P5/P6/P7/M11/M13/M14/M15） | 🔄 进行中 | M14、M7、M11、M1(五页全收口)、M2(AlertTable) 已完成（rev30–40）；剩 M2(其他表)/M3+P4/P5/P6/P7/M13/M15 |
...
| 批 2 · M2 表格排序（全站零 sorter） | 🔄 进行中 | AlertTable 四列（主机/级别/状态/触发时间）已加前端本地排序（rev40）；1 用例绿；变异（去 created_at sorter）红在行顺序断言。**剩 AssetTable/TicketTable/Runbook/Oncall/MetricSnapshot 等表格** |
...
5. ~~批 2 启动~~ → **M14（rev30）+ M7（rev31）+ M11 三处（rev32/33/34）+ M1 五页（rev35–39）+ M2 AlertTable（rev40）已完成**；剩 M2 排序（其他表）、M3 分页+P4 表格 memo（同批）+P5 服务端分页+P6 进度节流、M13 移动端、M15 空态/加载态。
