...
1.15. **M25 工单经手历史 + `resolved_at` 随状态收口（2026-09-11 需求文档与审查完成，实现未开始）** —— 需求文档 `docs/FIX-PLAN-TICKET-HISTORY.md`
   ...
   - 步骤 2c 门禁：`gofmt -l` 干净、`go vet ./...` 无输出、`go test -count=1 ./...` 27 包全 ok、`internal/service` 覆盖率 84.8%；前端 0 改动故 tsc/lint/vitest 未跑（如实记录）。**下一步**：步骤 3（`Create` 出生事件 —— **每次尝试各开一个事务**，出生历史行写在同一事务里，绝不在长事务里循环重试；真 PG 制造 `ticket_number` 冲突验自愈 R-5）。
