...
1.15. **M25 工单经手历史 + `resolved_at` 随状态收口（2026-09-11 需求文档与审查完成，实现未开始）** —— 需求文档 `docs/FIX-PLAN-TICKET-HISTORY.md`
   ...
   - **rev65 轮的量**：0 行产品代码，1 个新迁移（+ 1 down），1 个新 dbsmoke 用例 + Down 链 1 条正向断言 + 白名单 1 处、`autoMigrate()`/`liveModels()` 各 1 行。门禁：`gofmt -l` 干净、`go vet ./...` 无输出、`go test -count=1 ./...` 27 包全 ok、真 PG 冒烟两轮全绿且确认新用例**真跑**（T-42 假绿已排除）。**前端 0 改动**故 tsc/lint/vitest 未跑（如实记录）。**下一步**：步骤 2（`Actor` + `Update` 签名变更 + 事务/行锁 + 原始行 map diff）。
