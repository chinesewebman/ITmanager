...
1.15. **M25 工单经手历史 + `resolved_at` 随状态收口（2026-09-11 需求文档与审查完成，实现未开始）** —— 需求文档 `docs/FIX-PLAN-TICKET-HISTORY.md`
   ...
   - 门禁：`gofmt -l` 干净、`go vet ./...` 无输出、`go test -count=1 ./...` 27 包全 ok（`internal/api/handlers` 单跑亦 ok）、`internal/service` 覆盖率 84.9%；签名变更不触 DB 与前端，真 PG 冒烟与 tsc/lint/vitest 未跑（如实记录）。**下一步**：步骤 2c（`Update` 事务化 + `clause.Locking` 行锁 + 原始行 map pre/post diff + 批量插历史 + `CreateFromAlert`）。
