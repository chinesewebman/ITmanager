...
| 批 2（M1/M2/M3+P4/P5/P6/P7/M11/M13/M14/M15） | 🔄 进行中 | M14、M7、M11、M1(Oncall+AlertSuppressions+Topology) 已完成（rev30–37）；剩 M1(其他页)/M2/M3+P4/P5/P6/P7/M13/M15 |
...
| 批 2 · M1 标题体系统一（Oncall + AlertSuppressions + Topology） | 🔄 进行中 | Oncall Tabs 页补 PageHeader「值班管理」、AlertSuppressions 补 PageHeader「告警抑制」+ 按钮入 extra、Topology 补 PageHeader「网络拓扑」（h4）；各 1 用例绿；变异均红在 heading 断言。**剩 Runbook/Settings（AssetTimeline 子页 + MetricSnapshot 带 Tag 待定）** |
...
5. ~~批 2 启动~~ → **M14（rev30）+ M7（rev31）+ M11 三处（rev32/33/34）+ M1 Oncall（rev35）+ M1 AlertSuppressions（rev36）+ M1 Topology（rev37）已完成**；剩 M1 标题（Runbook/Settings）、M2 排序、M3 分页+P4 表格 memo（同批）+P5 服务端分页+P6 进度节流、M13 移动端、M15 空态/加载态。
