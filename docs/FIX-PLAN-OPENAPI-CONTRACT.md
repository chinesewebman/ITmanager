...
| 变异 | 施加 | 旧三条用例 | `公开端点集合相等`（新） |
|---|---|---|---|
| M31-M5 | 删 `/auth/logout` 的 `security: []`（公开→声明需认证） | 全 PASS | **FAIL**（该公开未公开） |
| M31-M6 | 给受保护端点 `/auth/me` 加 `security: []`（需认证→声明公开） | 全 PASS | **FAIL**（不该公开却公开） |
| M31-M7 | 删顶层 `security:`（D-5 的默认声明） | 全 PASS | **FAIL**（顶层缺 security） |

三条都只被新用例抓到，且两个方向各自有独立证据（M5 一个方向、M6 另一个方向）——
这正是 R-5 说的「集合相等管不到 `security:` 的值」的补位。
...
- ⚠️ **步 5 的漂移检查一度是空转的**（见 §8.5 R-7）：命令写的是
  `git diff -- frontend/src/services/api.types.ts`，但当时 cwd 已在 `frontend/`，该 pathspec
  相对仓库根不存在 → 匹配零文件、输出为空，被误读成「零漂移」。实际 `/auth/logout` 的
  `description` 变更**进了生成物**。**判据修正**：检查恒在仓库根跑，路径一律写全
  `frontend/src/services/api.types.ts`；或直接断言 `npm run gen:api` 后
  `git diff --exit-code` 的**退出码**（0/1），不要靠「输出为空」下结论。
### 8.3 门禁（含 `validate:api` 与 `gen:api` 漂移检查的首次实跑输出）
### 8.4 实现后审计与处置
...
- ~~**R-5：鉴权语义声明尚无守门用例**~~ → **已在步 6 结案**（`TestRoutes_OpenAPI公开端点集合相等`，
  见 §8.2 的 M31-M5..M7）。运行时侧判据：无凭据请求 **401 = 需要认证**；前提（受保护端点都在
  `AuthMiddleware` 层用 401 拒绝）由一条 `GET /assets` 钉子显式钉住 —— 将来若有公开端点自行返回 401，
  会先撞在钉子上而非静默误判。
- **R-7（步 6 自查发现，已补提交）**：步 5 的 `gen:api` 漂移检查**空转**——cwd 在 `frontend/` 时
  写了相对仓库根的 `src/services/api.types.ts`，匹配零文件、输出为空，被当成「零漂移」，
  于是 `90195af` 只提交了 `openapi.yaml`，**漏掉生成物** → 该 commit 上 CI 的漂移门禁是**红的**。
  已用 `fix(M31)` 单独补提交生成物（不 amend：`90195af` 已推送，改写历史要 force-push，禁用）。
  教训见 §8.3 的判据修正：**空输出 ≠ 无漂移**，要看退出码。
- **R-6（步 6 自查发现，已修）**：新用例第一版把运行时集合写成 `m[k] = (code != 401)`，
  受保护端点以 `false` 的形态留在**键**里，而差集循环遍历键 → 93 条全被算成公开。
  是新增的 `GET /assets` 前提钉子当场抓出来的。修法：只把公开的插进集合。
  **教训**：`map[string]bool` 当集合用时，「键存在」与「值为真」是两件事，混用会静默放大集合。
