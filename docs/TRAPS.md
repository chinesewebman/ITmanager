...
### T-57. `map[string]bool` 当集合用时, **"键存在" 与 "值为真" 是两件事** —— 填充写 `m[k] = cond` 会让全集进集合

**背景**: M31 新增 `TestRoutes_OpenAPI公开端点集合相等`, 运行时侧要构造
"无凭据可达的端点" 集合。第一版写成:
...
for k := range 真实路由集合(t, r) {
    ...
    r.ServeHTTP(w, req)
    运行时公开[k] = w.Code != http.StatusUnauthorized   // ← 全部 93 条都成了键
}
```

**现象**: 差集循环写的是 `for k := range 运行时公开 { if !spec公开[k] { 不该… } }` ——
遍历的是**键**, 而受保护端点也**有键**(值为 `false`)。于是 93 条全被算成"公开",
报出 90 条假失败, 而**真正的缺陷(漏标 `security: []`)会被淹没在噪音里**。

**检测线索**: 凡是把 `map[K]bool` 当**集合**用的地方, 问两句:
① 填充是 `m[k] = true`(只放成员) 还是 `m[k] = cond`(全集都进)?
② 消费是 `for k := range m`(只看键) 还是 `if m[k]`(看值)?
两者**必须配套**。混用不报错、不 panic —— 集合被静默放大到全集, 断言看起来"跑了"。

**解法**: ① 填充只放成员: `if cond { m[k] = true }`; ② 更稳的是给集合加一条
**前提钉子** —— M31 用 `assert.False(t, 运行时公开["GET /assets"], "GET /assets 无凭据竟非 401
—— 「非 401 即公开」的前提被破坏")`。这条钉子在写错的当天就把它抓了出来(全绿变红且指向明确),
是"哨兵断言"的一个正面样本: **不要只断言集合相等, 还要钉住集合里至少一个已知成员/非成员**。

**来源**: M31(2026-09-12, `docs/FIX-PLAN-OPENAPI-CONTRACT.md` §8.5 R-6)。同族: T-47(变异反证的
假信号 —— 断言没跑却像跑过)。

---

### T-58. **空输出 ≠ 无漂移** —— 检查命令自己空转时, 它长得和"通过"一模一样

**背景**: M31 D-2 要求"每个动 `openapi.yaml` 的 commit 都必须 `gen:api` 并提交生成物"
(CI 有硬漂移门禁)。步 5 的验收命令写成:

```bash
cd frontend && npm run gen:api >/dev/null && \
  git -C /root/work/itmanager diff --stat -- src/services/api.types.ts
```

**踩到的真相**: `git -C <repo>` 让 `git` 在仓库根执行, 但 `--` 之后的 pathspec 也**相对仓库根**解析,
而 `src/services/api.types.ts` 是相对 `frontend/` 的 → **匹配零个文件**。`git diff` 匹配不到东西时
**输出为空且退出码 0**, 与"预览无差异"的输出**完全一样**。于是被判成"零漂移", 而实际
`/auth/logout` 的 `description` 变更**确实进了生成物**(openapi-typescript 会把 description 编译成
JSDoc 注释 —— 改文案也会漂移, 不只改结构)。后果: 那个 commit 上 CI 的漂移门禁是**红的**。

**检测线索**: 任何"看输出空不空"的检查, 先问: ① 这条命令在**该 cwd** 下真的能匹配到文件吗?
② 它失败时的表现是"报错"还是"空输出"? 后者无法与成功区分。**空输出是所有判据里最像绿的一种红。**

**解法**: ① 判据改用**退出码**: `git diff --exit-code -- <path>`(0=无差异, 1=有差异),
让它**替你做判断**, 不要人眼看输出; ② pathspec 与 `-C` 必须同一基准 —— 要么都在仓库根写全路径
(`frontend/src/...`), 要么别用 `-C`; ③ 拿"已知有差异"的状态先跑一次, 确认检查**不是空转**的
(同 T-47 的自检思路)。 后续修正见 `docs/FIX-PLAN-OPENAPI-CONTRACT.md` §8.3。

**来源**: M31(2026-09-12, §8.5 R-7)。补提交见 `e73710c`(不 amend 已推送的 `90195af` —— 改写历史要 force-push, 项目禁用)。

---

### T-59. YAML 里 `description` **不能以反引号开头** —— 报错说"缩进错误", 病因却在保留字符

**背景**: M31 手工补 13 个 operation 时写了:

```yaml
    BulkResult:
      type: object
      description: `affected` 是**实际更新行数**, 可能小于传入条数。
```

**现象**: `swagger-cli validate` 报 `bad indentation of a mapping entry (3657:20)`, 箭头指向
`description` 那一行的冒号位置 —— 看起来像缩进/对齐问题, 但那一段的缩进**完全正确**。
真实病因: YAML 规范里 `` ` `` 和 `@` 是**保留指示符(reserved indicators)**, **不能作为 plain scalar
的首字符**。解析器在读到首字符时就判定"这不是一个合法 scalar", 于是把它当结构错误报出来,
错误位置落在冒号附近, 把人引向缩进。

**检测线索**: YAML 报"缩进 / mapping entry"错误但目视缩进无误时, 检查该 scalar 的**首字符**
是不是 `` ` `` / `@` / `*` / `&` / `!` / `%` / `{` / `[` / `>` / `|` 等指示符。

**解法**: 加引号即可 —— `description: '`affected` 是…'`(单引号内反引号无需转义)。
**更省事的判据**: 本仓库的 YAML 大量以 `` `符号` `` 开头写 description, 一律**用引号包住**
或改用 `|` 块标量; 反正 `validate:api` 会在提交前抓到, 别靠肉眼。

**来源**: M31(2026-09-12, 步 3, `docs/FIX-PLAN-OPENAPI-CONTRACT.md` §8.3)。
