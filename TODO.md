...
- [ ] **G-7 gin 受信代理未配置 → 限流可绕过、审计 IP 可伪造**（2026-09-09 新增，安全审计 F1①）— 未调 `SetTrustedProxies`，gin 默认信任全部来源（`0.0.0.0/0, ::/0`），`ClientIP()` 取 X-Forwarded-For **最左值**。后果：① 登录限流桶键含 `ClientIP()`，攻击者每请求换一个 XFF 即得无限额度（F1① 实测）；② 审计行 IP 变成攻击者可控字段，取证价值归零。**不能简单 `SetTrustedProxies(nil)`**：`docker-compose.yml` 里 nginx（`nmp-web`）与后端（`nmp-api`）是**不同容器**，后端看到的对端是 nginx 容器 IP，信任全关 → 全站共用一个限流桶（一个攻击者可锁死所有人登录）+ 审计 IP 全变容器 IP。待办：① config 增 `server.trusted_proxies []string`（显式 CIDR，默认空=不信任任何来源并 WARN）；② compose 给该网络定 ipam 固定子网，配置写入该子网；③ 测试：对端不可信时 XFF 不影响 `ClientIP()`；对端可信时取最右不可信跳
...
- [ ] **G-6 Web 界面 TLS 最低版本无强制、无验证**（2026-09-09 新增，PCI DSS 4.2.1）— 仓库内**没有任何 TLS 终止或最低版本约束**：`frontend/nginx.conf` 只有 `listen 80`（无 `ssl_*`、无 HSTS）、Go 后端 `cmd/server/main.go:104` 是明文 `ListenAndServe()`；`ssl_protocols TLSv1.2 TLSv1.3` 只存在于设计文档 `network-monitor-design.md:6844`。当前「只支持 TLS 1.2+」**完全依赖外部反代/负载均衡的配置，无法自证**。待办：① `08-部署运维.md` 写明 TLS 终止点与最低版本（PCI DSS 4.2.1）；② 提供 `scripts/check-tls.sh`（openssl s_client 断言 TLS1.0/1.1 握手失败、1.2/1.3 成功）；③ 若用仓库自带 nginx，补 443 + `ssl_protocols TLSv1.2 TLSv1.3` + HSTS + 80→443 跳转模板
