...
- [x] **G-25 GLPI 同步一次新增 ≥2 张工单必失败（`ticket_number` 撞号）**（2026-09-09 新增，G-22 修复实测暴露）— **已修（2026-09-10）**。原状：`models/ticket.go` 的 `Ticket.BeforeCreate → generateTicketNumber` 按「**当天已建条数**」算号（`TICKET-YYYYMMDD-<序号>`），而 gorm 的 `before_create` 回调对整片 slice 的**每一行都跑完才进 INSERT** → 每行查到的条数相同 → 整批拿到同一个号 → `tickets.ticket_number` 唯一索引拒绝整批，`SyncFromGLPI` 返回 error、`SyncAll` 记 glpi 失败。**实测**：一次 `CreateInBatches` 2 张票 → 同一个 INSERT 里两行都是 `TICKET-20260910-A` → `UNIQUE constraint failed`；逐条 `Create` 正常（每行插完再算下一行）。影响面：GLPI 首次同步（或任何新增 ≥2 张票的同步）全部失败。
  ...
  - **残余风险（未修，已认知）**：预分配与逐条路径同源，仍是 count-based —— **删过工单**后 count 回退会算出已占用的号（逐条路径靠 `TicketService.Create` 的 5 次重试兜底，但那 5 次算出的号相同 → 恒 409）。属 D-2 家族的另一条分支，非本轮范围。
...
- [ ] **G-31 同类错误回显点未逐一收口（HTTP 400 文案 / 上游响应体 / panic 值）**（2026-09-09 新增，G-28 审查发现，未修）— G-28 只修了**已核实带凭据**的四处出口，同类写法还有：① `apierr.BadRequest(c, "...: "+err.Error())` 全仓约 30 处（`grep -rn 'BadRequest(c, .*err.Error()' internal/`），绝大多数是 DB/参数校验错误（不含 URL），但**形态相同**——将来任何一处接上第三方客户端错误就会复现 G-28；② ~~`internal/httpx/httpx.go:167` 的 4xx 分支把**上游响应体原文**拼进错误~~（**2026-09-09 晚已闭合**：httpx 出口统一过 `redact.Text`，见 G-28 第二轮 ①）（`httpx: %s %s → %d: %s`），上游若回显了凭据（例如把请求 URL/DSN 放进错误 JSON）会连带落日志；③ `middleware/recovery.go` 的 `slog.Any("panic", err)` 把 panic 的 error 值整块打出（当前无带凭据的 panic 源）。待办：把「错误文本进日志/响应」收敛到一个统一出口（例如 `apierr` 里集中脱敏 + httpx 只留状态码与截断后的摘要），或用 lint/测试钉住「新调用点必须过 redact」。 **补充（2026-09-09，G-33 M1 rev3 正确性审计 L-1）**：`ErrInvalidInput` 的哨兵文本是英文 `invalid input`，凡 `BadRequest(c, err.Error())` 的路径用户都会看到 `invalid input: 渠道名称不能为空` 这类中英混排（10+ handler 同形）。属本条的「错误文案」面，随统一出口一并处理；单独改共享哨兵会波及全仓文案断言，故不在 G-33 轮动。 **补充（2026-09-09，G-33 M3 安全审计 L-2/L-3）**：① `worker.go:157` 的发送错误日志只过 `redact.Text`、**未过 `stripControlChars`**（控制字符净化只在 `markFailed` 的入库链上）→ 异常 SMTP 服务端/代理文本带 CR/LF 可伪造行式消费的应用日志行（DB 侧已被挡住，无 22021 风险）；修法是日志出口复用同一口径（`sanitizeSnippet` 或 `stripControlChars`）。② 前端 `Settings.tsx` 除渠道保存外的 8 处 `console.error(..., error)` 仍整体打印 axios error，其 `error.config.data` 是请求体明文（NetBox/GLPI/Zabbix 的 token、API Key）——渠道保存那处已在 M3 修（只记 `status` + 后端脱敏 `message`），其余按同一写法收敛。
